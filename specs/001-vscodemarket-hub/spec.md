...
#### 宪法引用与 ADR 触发
- **FR-CONST-001**: 本规格与 `.specify/memory/constitution.md` v0.2.0 强对齐;
  任何后续 PR 试图撤销 / 弱化宪法 IV(极简立场)所列"当前未启用项"(限频 /
  反作弊 / 验证码 / 用户系统 / 独立文档站),必须在 PR 中给出:
  (a) 触发条件(具体流量 / 滥用 / 合规要求),
  (b) 与所选替代方案的对比(至少 2 个,说明为什么更简方案不可行),
  (c) 由维护者评审通过后方可合入;合入即触发宪法 PATCH 升级。
  本规格 §7 范围外声明不构成上述流程的豁免。
